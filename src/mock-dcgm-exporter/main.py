...
def _build_metrics() -> str:
    ...
    for n in range(NODE_COUNT):
        for g in range(GPUS_PER_NODE):
            ...
            util_by_gpu[(n, g)] = util

            total_mem = 40960  # 40 GiB A100
            used = 4096 + (util / 100) * 36000 + random.gauss(0, 200)
            fb_used_by_gpu[(n, g)] = max(0.0, min(total_mem, used))
    ...
    for n in range(NODE_COUNT):
        for g in range(GPUS_PER_NODE):
            lines.append(f'DCGM_FI_DEV_FB_USED{{{_gpu_label(n, g)}}} {fb_used_by_gpu[(n, g)]:.0f}')

    metric("DCGM_FI_DEV_FB_FREE", "Frame buffer memory free (in MiB).")
    for n in range(NODE_COUNT):
        for g in range(GPUS_PER_NODE):
            total = 40960  # 40 GiB A100
            free = total - fb_used_by_gpu[(n, g)]
            lines.append(f'DCGM_FI_DEV_FB_FREE{{{_gpu_label(n, g)}}} {free:.0f}')

    metric("DCGM_FI_DEV_GPU_TEMP", "GPU temperature (in C).")
    for n in range(NODE_COUNT):
        for g in range(GPUS_PER_NODE):
            util = util_by_gpu[(n, g)]
