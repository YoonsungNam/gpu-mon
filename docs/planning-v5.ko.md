...
> **기존 환경**: Baremetal - Zabbix + DCGM, VMware vSphere GPU VM, Internal Batch Scheduler (IBS)
> **핵심 원칙**:
...
> - 레거시 시스템 메타데이터(VMware VM, IBS Job/Node/Project/Pool)를 수집하여 GPU 메트릭과 결합(Enrichment)
...
| 아키텍처 다이어그램 전면 개정 — IBS/VMware는 Data Sources에 통합, 경로 A(Pull)/B(Push) 표기 | 5 |
| DCGM Profiling ↔ CUPTI 충돌 관리 전략 추가 | 3 |
| IBS 메타데이터 저장 전략 3분류 (시계열/현재값/스냅샷), ibs_projects/ibs_pools 테이블 추가 | 4, 8 |
...
| 변경 사항 | 영향 섹션 |
|---|---|
| [신규] 섹션 4: 레거시 시스템 메타데이터 통합 — VMware vCenter GPU VM Inventory, Internal Batch Scheduler (IBS) Job/Node 메타데이터 수집 체계 | 4 (신규) |
...
| ClickHouse 테이블 추가 (`ibs_jobs`, `ibs_nodes`, `vmware_vm_inventory`) | 8 |
...
| **Legacy Metadata** *(v4 신규)* | VMware VM Inventory, IBS Job 메타데이터, IBS Node 상태 | **ClickHouse** | Snapshot 기반 이력 관리, GPU 메트릭과 JOIN 분석에 최적 |
...
─────────────────              ─────────────────
GPU 0: Util 85%                 "사용자 A의 LLaMA 학습 Job이
GPU 0: Temp 72°C                 IBS queue 'high-priority'에서
...
| **Metadata Collector** | IBS + VMware API 폴링 → ClickHouse | K8s Deployment | v4 동일 |
...
| **B: DCGM Job Stats** | 없음 | ❌ 불가 | 낮음 | Phase 3 (IBS 연동 시 함께) |
...
```
Phase 3 (즉시):
  • DCGM Job Stats로 IBS Job별 GPU 효율 요약 수집
  • L2 Profiling과 충돌 없이 Job 수준 통계 확보
  • IBS Job lifecycle hook으로 DCGM stats start/stop 연동
...
| **L2.5 Job 통계** | Job별 GPU Util/Mem 집계 | ✅ | ✅ | **DCGM Job Stats** | ClickHouse | **IBS 연동 (충돌 없음)** |
...
IBS는 Internal Batch Scheduler의 약자입니다. Slurm과 유사한 역할을 하며, Job 제출, 스케줄링, 실행, 완료를 관리합니다.
...
```
방법 1: IBS REST API 폴링 (권장)
  IBS가 REST API를 제공하는 경우, Metadata Collector가 주기적으로 API를 호출.
  
  예상 엔드포인트 (IBS API 스펙에 따라 조정):
...
방법 2: IBS CLI 파싱
  IBS CLI (ibsjobs, ibsnodes 등)의 출력을 파싱하여 구조화.
  Metadata Collector가 SSH 또는 로컬에서 CLI 실행.

  예: ibsjobs --format=json --state=running
      ibsnodes --format=json
      ibsprojects --format=json
      ibspools --format=json

방법 3: IBS DB 직접 조회 (fallback)
  IBS 백엔드 DB에 read-only 접근하여 쿼리.
...
| 데이터 | 설명 | 활용 |
|---|---|---|
| **VM Inventory** | VM 이름, UUID, 상태, 생성일, 연락처(annotation) | GPU VM 현황 파악, 인벤토리 관리 |
...
| 소스 | 데이터 | 비고 |
|---|---|---|
| **LDAP/AD** | 사용자→부서/팀 매핑 | IBS 사용자 ID를 조직 정보로 확장 |
...
│  ┌───────────────────────────────────────────────────────────┐  │
│  │                      Scheduler                             │  │
│  │  IBS Jobs:       매 60초 (running/pending) — 시계열         │  │
│  │  IBS Jobs:       매 300초 (recently_completed) — 시계열     │  │
│  │  IBS Nodes:      매 120초 — 현재값 (ReplacingMT)           │  │
│  │  IBS Projects:   매 600초 — 스냅샷 (변경 드묾)             │  │
│  │  IBS Pools:      매 600초 — 스냅샷 (변경 드묾)             │  │
...
| **IBS 연동** | HTTP client / subprocess | API 방식 또는 CLI 출력 파싱 |
...
| **컨테이너 이미지** | Alpine/Distroless 기반 | 경량 이미지, 컨테이너 레지스트리로 push |
...
```
IBS Job 데이터 [시계열, MergeTree]:
  ├── Running/Pending Job: 60초마다 → 시계열 이력 전체 보관
  ├── Completed Job: 완료 후 추가 기록
  └── 보관: 6개월 (TTL) — Job 대기 시간, GPU-Hours 분석에 활용

IBS Node 데이터 [현재값, ReplacingMergeTree]:
  ├── 120초마다 폴링 → 노드당 최신 1건만 유지
  ├── 이력 필요 시 ibs_jobs에서 간접 추적 또는 gpu_events에 이벤트 기록
  └── TTL 없음 (자동 dedup으로 행 수 안정적)

IBS Project/Pool 데이터 [스냅샷, ReplacingMergeTree]:
...
이력 분석 예시:
  "지난 3개월간 팀 A의 GPU 사용 패턴 변화" → ibs_jobs 시계열
  "특정 Job의 대기 시간이 얼마였는지" → ibs_jobs pending→running 전환
...
핵심 인사이트:
  ...
  → IBS가 "Job #84723 → gpu-node-03의 GPU [0,1,2,3]" 을 알려주면
...
IBS가 각 Job에 GPU를 exclusive로 할당하므로, per-GPU 메트릭 = per-Job 메트릭입니다.
...
방법 A: NVML Accounting Mode (권장, 오버헤드 ~0%)
  ...
  IBS Job의 PID 정보와 매핑:
    ibs_jobs.metadata JSON에 {"pid": 12345} 포함 시
    → gpu_process_util과 ibs_jobs를 PID 기준 JOIN

방법 B: DCGM Job Stats API (스케줄러 연동)
  ────────────────────────────────────────
  DCGM에 내장된 Job 수준 통계 기능입니다.
  IBS가 Job 시작/종료 시 DCGM에 알려주면, DCGM이 해당 기간의
  GPU 사용 통계를 집계합니다.
  
  연동 흐름:
    Job 시작 → IBS Hook → dcgmi stats -s <group_id>  (통계 수집 시작)
    Job 종료 → IBS Hook → dcgmi stats -x <group_id>  (통계 수집 종료)
    → 결과: 해당 Job 기간의 GPU Util, Memory, ECC 에러, SM Occupancy 등
  
  장점: GPU Util뿐 아니라 SM Clock, Memory 등 종합 통계 제공
  단점: IBS의 Job lifecycle hook이 필요 (스케줄러 운영 담당자와 협의 필요)
...
─────────────────────────────────────
  • IBS Job 메타데이터의 (node_list, gpu_indices)와
...
─────────────────────────
  ...
  선택 B: DCGM Job Stats 연동 (IBS Hook 필요)
    • IBS Job 시작/종료 시 DCGM에 통계 수집 시작/종료 신호
...
│  user: user-a   │  meta)  │  GPU 2: Util 91%   │  →VM)  │             │
│  team: team-a   │        │  GPU 3: Util 90%   │        │ Query B:    │
│                  │        │                    │        │ VictoriaM.  │
...
│  │ (IBS Batch Scheduler)     │ │                    │ │ (vCenter 관리)      │ │
│  │                           │ │                    │ │                      │ │
...
│                                                                                │
│  ┌────────────────────────────────┐                                           │
│  │  Metadata Collector             │  ← IBS API + vCenter API 폴링            │
...
│  │ 표준 라벨: env / cluster /  │   │ • ibs_jobs / ibs_nodes                   │ │
│  │ node / gpu / workload_type  │   │ • ibs_projects / ibs_pools               │ │
│  │                             │   │ • vmware_vm_inventory                   │ │
│  └──────────────┬──────────────┘   └─────────────────┬───────────────────────┘ │
│                 │                                     │                         │
└─────────────────┼─────────────────────────────────────┼─────────────────────────┘
...
| 컴포넌트 | 역할 | 배포 방식 | 비고 |
|---|---|---|---|
| **Metadata Collector** | IBS + VMware 메타데이터 수집 → ClickHouse | K8s Deployment (1 replica) | Adapter 패턴, Python/Go |
| ├─ IBS Adapter | IBS Job/Node 메타데이터 폴링 | Collector 내장 모듈 | REST API 또는 CLI 방식 |
...
|---|---|---|---|---|
| **DCGM Job Stats** | Job별 GPU 효율 집계 (L2.5) | IBS Hook 연동 | ✅ 없음 | Phase 3, L2와 동시 가능 |
...
```
  IBS API/CLI 서버                          VMware vCenter API
...
### 8.6 IBS 메타데이터 저장 전략 개요 (v4.1 개정)

IBS 관련 데이터를 **데이터의 성격에 따라 3가지 저장 전략**으로 분리합니다.
...
│  • Nodes: 현재 상태만 중요, 이력은 ibs_jobs에서 간접 추적 가능       │
...
### 8.7 IBS Job 메타데이터 테이블 — 시계열 (v4.1)
...
CREATE TABLE ibs_jobs (
    ...
    -- Job 식별
    job_id           String,                -- IBS Job 고유 ID
    job_name         String,                -- Job 이름 (사용자 지정)

    -- 사용자/조직 정보
    user_id          LowCardinality(String),  -- 제출자 ID
    team             LowCardinality(String),  -- 팀/그룹
    project          LowCardinality(String),  -- IBS Project 이름 (FairShare 단위)
    queue            LowCardinality(String),  -- IBS Queue (파티션) 이름
...
### 8.8 IBS Node 상태 테이블 — 현재값 (v4.1)

**저장 전략: 현재값 (ReplacingMergeTree)** — 노드당 최신 상태 1건만 유지합니다. 과거 이력이 필요할 때는 ibs_jobs 테이블에서 간접 추적합니다.
...
ReplacingMergeTree:
  ...
  노드 상태 이력이 필요한 경우:
  → ibs_jobs 테이블에서 "이 노드에 할당된 Job 변화"로 간접 추적 가능
...
### 8.9 IBS Project 정보 테이블 — 스냅샷 (v4.1 신규)
...
CREATE TABLE ibs_projects (
    ...
    project_id       String,                  -- IBS Project 고유 ID/이름
    cluster_id       LowCardinality(String),  -- IBS 클러스터 식별자
    ...
    -- 전체 원본 (IBS API 응답 그대로)
    raw_config       String                   -- JSON: IBS API 원본 응답 전체
) ENGINE = ReplacingMergeTree(collected_at)
...
-- FairShare 대비 실제 GPU 사용률 (ibs_jobs와 결합)
...
### 8.10 IBS Pool 정보 테이블 — 스냅샷 (v4.1 신규)
...
| 7 | **`ibs_jobs`** | **MergeTree** | **시계열** | Metadata Collector | 6개월 | **Job 라이프사이클 이력** |
| 8 | **`ibs_nodes`** | **ReplacingMT** | **현재값** | Metadata Collector | - | **노드 최신 상태** |
| 9 | **`ibs_projects`** | **ReplacingMT** | **스냅샷** | Metadata Collector | - | **FairShare/Limit/License (JSON)** |
| 10 | **`ibs_pools`** | **ReplacingMT** | **스냅샷** | Metadata Collector | - | **Logical Node Pool 구성 (JSON)** |
...
**목표**: 수요 데이터/인벤토리 적재 + **레거시 메타데이터(IBS, VMware) 수집 및 GPU 메트릭 결합** + **DCGM Job Stats 연동**

- [ ] gpu_demand, gpu_inventory, gpu_events 테이블 생성
- [ ] **ibs_jobs, ibs_nodes, ibs_projects, ibs_pools, vmware_vm_inventory 테이블 생성 (v4.1)**
- [ ] **Metadata Collector 개발 (v4)**
  - [ ] IBS Jobs Adapter (시계열), IBS Nodes Adapter (현재값)
  - [ ] IBS Projects/Pools Adapter (스냅샷, JSON)
...
- [ ] **DCGM Job Stats 연동 (L2.5, CUPTI 충돌 없음)**
  - [ ] IBS Job lifecycle hook 설계 (Job 시작 → dcgmi stats -s / Job 종료 → dcgmi stats -x)
...
- [ ] VM 환경 에이전트 배포
- [ ] **Job Explorer 대시보드 구축**: IBS Job↔GPU 메트릭 결합 + DCGM Job Stats
- [ ] **VM GPU Inventory 대시보드 구축**: VMware VM 현황

**완료 기준**: Grafana에서 "gpu-node-03의 GPU 0"에서 실행 중인 IBS Job 정보 확인 가능, Job별 GPU Util 요약 확인 가능, VMware GPU VM 인벤토리 조회 가능
...
| ★ **Job Explorer** *(v4)* | **ClickHouse + VM** | - | **IBS Job↔GPU 매핑, 팀별/사용자별 GPU 사용률, Job 대기 시간, Queue 현황** |
...
| 패널 | 데이터 소스 | 설명 |
|---|---|---|
| Running Jobs 테이블 | ClickHouse (ibs_jobs) | Job ID, 이름, 사용자, 팀, 노드, GPU수, 경과 시간 |
| Job별 GPU Util Heatmap | VM + ClickHouse Join | 각 Job이 사용하는 GPU들의 평균 Util |
| 팀별 GPU 점유 Bar Chart | ClickHouse (ibs_jobs) | 팀별 현재 GPU 할당 수 |
| Queue 대기 Job 수 | ClickHouse (ibs_jobs) | pending 상태 Job 수 by queue |
| 노드 상태 Map | ClickHouse (ibs_nodes) | idle/alloc/drain/down 시각화 |
| Job 이력 Timeline | ClickHouse (ibs_jobs) | 특정 노드/GPU의 Job 실행 이력 |
...
| **D12** | **메타데이터 수집 방식** *(v4)* | **에이전트 기반 vs 중앙 폴링** | **중앙 폴링 (Metadata Collector)** | **노드에 에이전트 추가 없이, 1개 Deployment로 전체 메타데이터 수집. IBS/vCenter는 API 서버가 있으므로 폴링이 적합** |
...
| **D16** | **IBS 메타데이터 저장 전략** *(v4.1)* | **단일 전략 vs 데이터별 분리** | **3가지 전략 분리** | **Jobs: 시계열(MergeTree)로 라이프사이클 추적, Nodes: 현재값(ReplacingMT)으로 최신만, Projects/Pools: 스냅샷(ReplacingMT, JSON)으로 설정 변경 기록** |
...
│                                                                       │
│  레거시 메타데이터 수집 (v4~)                                         │
│  └── Metadata Collector  - IBS + VMware API 폴링 → ClickHouse         │
│      ├── IBS Adapters     - Jobs(시계열), Nodes(현재값),              │
...
│  └── DCGM Job Stats     - IBS Job hook 연동, L2와 동시 가능          │
│                                                                       │
...
| **IBS API 스펙** | REST API 엔드포인트, 인증 방식, 응답 형식 | 스케줄러 운영 담당자 확인 |
| **IBS CLI 사용 가능 여부** | API가 없을 경우 CLI 출력 형식 확인 | 스케줄러 운영 담당자 확인 |
...
| **IBS Job↔GPU 인덱스 매핑** | IBS가 어떤 형태로 GPU 인덱스를 제공하는지 | 스케줄러 운영 담당자 확인 |
| **네트워크 접근성** | K8s 클러스터 → IBS API, vCenter 간 방화벽 | 플랫폼 운영 담당자 확인 |
